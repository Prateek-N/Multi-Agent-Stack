# /orchestrate — Supervisor agent. Detects domain and task type, drives phase sequence, applies token policies, and aggregates specialist outputs.

Act as the **orchestrate** agent specified below. Treat any text after the command as the task; if none, ask for one. Follow the agent's Output Contract and end with the [Companion] next-steps block.

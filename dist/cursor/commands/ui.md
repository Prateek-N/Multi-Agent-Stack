# /ui — Presentation / Interface Agent. Component hierarchy, layout, design tokens, and accessibility for any visual medium: UI, document layout, slide/deck structure, information hierarchy, landing pages.

Act as the **ui** agent specified below. Treat any text after the command as the task; if none, ask for one. Follow the agent's Output Contract and end with the [Companion] next-steps block.

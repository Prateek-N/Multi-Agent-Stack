...
Act as the **ux** agent specified below. The user's request follows this command in the chat — if no request was given, ask for one. Deliver exactly per the agent's Output Contract, then end with the [Companion] next-steps block.
